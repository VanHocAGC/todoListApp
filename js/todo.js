...
async function checkLogged() {
  if (!isLive) {
    ...
    window.location.replace(
      "https://vanhocagc.github.io/todoListApp/login.html"
    );
  } else {
    showSuccess(`Xin chào ${userWorking}`);
    document.querySelector(
      ".user-infor"
    ).innerHTML = `<p>${userWorking}</p> <img src="./image/undraw_profile_pic_ic5t.svg" alt="user infor"><button onclick="showLogout()"><i class="fas fa-chevron-down"></i></button>`;
  }
}
...
function showTask() {
  ...
  let backgroundColor;
  let chooseToCount =
    parseInt(currentPage) * 4 > userNow.taskList.length
      ? userNow.taskList.length
  ...
  for (let i = (parseInt(currentPage) - 1) * 4; i < chooseToCount; i++) {
    ...
    if (userNow.taskList[i].status === "Tạm dừng") {
      ...
      new Date(userNow.taskList[i].deadline).getTime() + 86400000 <=
        new Date().getTime() &&
      userNow.taskList[i].status === "Đang làm"
    ) {
      color = "#ff384c";
...
function showPagination() {
  ...
  for (let i = 0; i < pageLength; i++) {
    if (i == 0) {
      paginationContent += `<div class="page active" onclick="beActive(this)">${
        i + 1
      }</div>`;
    } else {
      paginationContent += `<div class="page" onclick="beActive(this)">${
        i + 1
      }</div>`;
    }
  }
